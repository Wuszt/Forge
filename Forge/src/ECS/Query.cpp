...
void ecs::Query::VisitArchetypes( const VisitFunc& visitFunc ) const
{
	...
	ecs::CommandsQueue commandsQueue( m_ecsManager );
	...
	for ( const auto& archetype : m_ecsManager.m_archetypes )
	{
		...
		{
			m_ecsManager.TriggerOnBeforeReadingArchetype( *archetype, combinedFragmentFlags, commandsQueue );
			m_ecsManager.TriggerOnBeforeModifyingArchetype( *archetype, m_includedMutableFragments, commandsQueue );
		}
	}
