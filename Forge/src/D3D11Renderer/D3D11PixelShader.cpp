...
namespace d3d11
{
		...
		: m_context( context )
	{
		...
		if( errorMsg )
		{
			if( result == S_OK )
			{
				FORGE_LOG_WARNING( "Pixel Shader compilation warning: %s", static_cast<char*>( errorMsg->GetBufferPointer() ) );
			}
			else
			{
				FORGE_LOG_ERROR( "Pixel Shader compilation error: %s", static_cast<char*>( errorMsg->GetBufferPointer() ) );
			}
