...
{
	return std::make_unique< d3d11::D3D11VertexShader >( m_context, m_device, "Shaders\\" + path );
}

std::unique_ptr< const renderer::IPixelShader > d3d11::D3D11ShadersManager::CreatePixelShader( const std::string& path ) const
{
	return std::make_unique< d3d11::D3D11PixelShader >( m_context, m_device, "Shaders\\" + path );
}
